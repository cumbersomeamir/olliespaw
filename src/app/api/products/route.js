...
// Simple in-memory fallback data if DB is not configured
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ products: products.length > 0 ? products : FALLBACK_PRODUCTS }, { status: 200 });
  } catch (err) {
...
export async function POST(request) {
  try {
    ...
    const createdMany = await Product.insertMany(body, { ordered: false });
